...
cal_m_sd_IQR <- function(cor_matrix,ave_logcpm){
  ...
  for(i in 1:10){
    for(j in 1:10){
      ...
      if(i!=j){sd_cor_mat[i,j]=sd(cor_tmp);IQR_cor_mat[i,j]=IQR(cor_tmp)}
    }
    grp_mean=c(grp_mean,mean(ave_logcpm[grp_loc[[i]]]))
  }
  return(list(sd_cor_mat=sd_cor_mat,IQR_cor_mat=IQR_cor_mat,grp_mean=grp_mean))
}
...
qqplot_density2<-function(cor_matrix,i,j){
  ...
  if(i==j){cor_tmp=cor_tmp[upper.tri(cor_tmp)]}
  
  qq_=qqplot(cor_ref_vec,as.vector(cor_tmp),xlab="reference correlations - (9,9)",ylab=paste0("correlations in (",i,",",j,")"),cex.lab=1,cex.axis=1,cex=0.2)
  abline(0,1,col="blue")
}
