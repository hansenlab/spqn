...
rpm_filt_v2 <- function(counts) {
  counts_log2cpm <- log2(cpm(counts)+0.5)
  keep <- which(rowMedians(data.matrix(counts_log2cpm)) > 0) # in original code, counts is a tibble. Needs matrixStats
  counts_log2cpm_keep <- counts_log2cpm[keep,]
  counts_raw_keep <- counts_raw[keep,]
  return(list(counts_raw_keep=counts_raw_keep,
              counts_log2cpm_keep=counts_log2cpm_keep,
              keep=keep))
}

removePCs <- function(counts_logrpm, num_PCs = 4){
    if(num_PCs > 0) {
        counts_removePCs <- t(removePrincipalComponents( scale(t(counts_logrpm)), num_PCs)) # From WGCNA
        return(counts_removePCs)
    } else {
        return(counts_logrpm)
    }
}
