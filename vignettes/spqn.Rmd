...
To compare the distribution of correlation across different expression levels, we get the correlation matrix after sorting the genes by their expression level.
...
A number of studies predict the co-expressed genes by setting a threshold. We set 0.1% highest correlations as predicted signals in each bin. We compared the distibution of signals for bins on the diagonal. We find that the the predicted signals are stronger for high expressed genes and weaker for low expressed genes. Because of such bias, the true signals among low expressed genes could be hidden and may not be detected. 
```{r, message = FALSE, fig.width = 4, fig.height = 4}
plot_signal_condition_exp(cor_m, ave_logrpkm, 0.1)
```

Next we looked furtherly into the bins across the entire correlation matrix. We quantify the variance inside a bin using inter-quantile range (IQR), which is the distance from the 1st and 3rd quantile. We use the width of box in the plot to denote IQR. We observe that the increase of expression level for either gene would cause an increase of the IQR  
```{r, message=FALSE}
IQR_list <- get_IQR_condition_exp(cor_m, rowData(gtex.4k)$ave_logrpkm)
box_plot(IQR_list)
```

We also plot the marginal relationship between IQR of correlation and the expression level of the lower expression group for each bin, and we also see a positive relationship.
```{r, message = FALSE, fig.width = 4, fig.height = 4}
...
We furtherly examined the difference of the distributions using qqplot. We used the bins with the second highest expression level on the diagonal as reference, and compared the distribution with other bins in the diagonal. We observe that the distributions not only differs in the scale, but also in the shape. 
```{r, message = FALSE, fig.width = 4, fig.height = 4}
par(mfrow = c(3,3))
for(j in 1:10){
...
# Using SpQN
To avoid the bias in the co-expression analysis, we develop a normalization method for correlation matrix to remove the meam correlation relationship, named as spatial quantile normalization (SpQN). The main idea is to use bins to partition the correlation matrix with genes sorted by exp level, and then apply quantile normalization for each bin. To smooth the normalization, we used a larger bin to approximate the empirical distribution of the inner bin.  And we applied 1-dimension quantile normalization on the inner bins using its approximated distribution. The target distribution we used here is the bin with the second highest expression level on the diagonal. 

Using SpQN is relatively easy. In the function normalize_correlation, Parameter ngrp is the row/column number of bins that we use to partition the correlation matrix. For example, setting ngrp=20 means to partition the correlation matrix into 20 by 20 bins. Parameter size_grp is the size of the outter bins that used to appriximate the distribution of the inner bins, in order to smooth the normalization. Parameter ref_grp is the location of the reference bin on the diagonal, whose distribution will be used as target distribution in the normalization.
...
Distribution of correlations for bins on the diagonal of the correlation matrix.
```{r, message = FALSE, fig.width = 4, fig.height = 4}
plot_signal_condition_exp(cor_m_spqn, ave_logrpkm, 0)
```

Distribution of predicted signal and background correlations for bins on the diagonal of the correlation matrix.
```{r, message = FALSE, fig.width = 4, fig.height = 4}
plot_signal_condition_exp(cor_m_spqn, ave_logrpkm, 0.1)
```

IQR of bins across the entire correlation matrix.
```{r, message=FALSE}
IQR_spqn_list <- get_IQR_condition_exp(cor_m_spqn, rowData(gtex.4k)$ave_logrpkm)
box_plot(IQR_spqn_list)
```

Q-Qplot to compare the distribution for the bin with the 2nd highest expression level on the diagonal with other bins on the diagonal.
