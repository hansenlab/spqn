...
Calculate IQR of correlations for each expression level
```{r, message=FALSE}
IQR_list <- cal_m_sd_IQR(cor_m, rowData(gtex.4k)$ave_logrpkm)

```

Compare the IQR of correlations between genes in each expression level
...
QQplots
Compare the distribution of each bin and the reference bin
...
min-IQR relationship
relationship between the lowest of the two expression bins and IQR 
```{r, message = FALSE, fig.width = 4, fig.height = 4}
...
To correct the mean-correlation dependency, applying SPQN in the correlation matrix, such that correlations of genes follows approximately the same distribution across different expression levels.
```{r spqn, message = FALSE, fig.width = 4, fig.height = 4}
cor_m_spqn <- quantile_norm(cor_m, ngrp=20, size_grp=300, ref_grp=18)
```

Check the mean-correlation dependency after applying SPQN.
Ridgeplot for diagonal
```{r, message = F, fig.width = 4, fig.height = 4}
plot_diagonal_ridge4(cor_m_spqn)
```
...
```{r, message=FALSE}
IQR_spqn_list <- cal_m_sd_IQR(cor_m_spqn, rowData(gtex.4k)$ave_logrpkm)
...
```{r, message = FALSE, fig.width = 3, fig.height = 3}
for(j in 1:10){
    qqplot_density2(cor_m_spqn, j, j)
}
```
