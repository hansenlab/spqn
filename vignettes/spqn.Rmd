...
plot_signal_condition_exp(cor_m, 0)
```
...
boxplot(IQR_list$IQR_cor_mat, IQR_list$grp_mean)
```
...
```{r, message = F, fig.width = 4, fig.height = 4}
plot_signal_condition_exp(cor_m_spqn, 0)
```
...
boxplot(IQR_spqn_list$IQR_cor_mat,IQR_spqn_list$grp_mean)
```
