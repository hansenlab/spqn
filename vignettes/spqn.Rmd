...
```{r, message=FALSE}
IQR_list <- get_IQR(cor_m, rowData(gtex.4k)$ave_logrpkm)
...
```{r, message = F, fig.width = 4, fig.height = 4}
plot_diagonal_ridge(cor_m_spqn)
```
...
Calculate IQR of correlations for each expression level
```{r, message=FALSE}
IQR_spqn_list <- get_IQR(cor_m_spqn, rowData(gtex.4k)$ave_logrpkm)
